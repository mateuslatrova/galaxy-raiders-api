...
open class Object2D(
  ...
  fun setCenter(newCenter: Point2D) {
    this.center = newCenter
  }

  fun setVelocity(newVelocity: Vector2D) {
  ...
  /**
   * Calculates the variation of velocity between two objects in a collision. This variations
   * affects only the direction of impact.
   *
   * @param target Another space object we are colliding with.
   *
   * @see https://arxiv.org/abs/1909.10053
  ...
  /**
   * Calculates the impulse, i.e., the variation of momentum in a collision with another space
   * object.
   *
   * @param target Another space object we are colliding with.
   * @param coefficientRestitution Ratio of conservation of momentum during the collision.
  ...
  private fun calculateImpactImpulse(
    ...
    return (((this.mass * target.mass) / (this.mass + target.mass)) *
        (1 + coefficientRestitution) *
        deltaVelocity)
  }
}
