...
data class SpaceField(val width: Int, val height: Int, val generator: RandomGenerator) {
  ...
  fun generateExplosion(asteroid: Asteroid, missile: Missile) {
    var explosion: Explosion = Explosion(asteroid, missile)
    explosion.trigger()
  ...
  fun trimExplosions() {
    this.explosions = this.explosions.filter { it.inBoundaries(this.boundaryX, this.boundaryY) }
  }  

  fun triggerNewExplosions() {
    for (missile in this.missiles) {
      if (this.wasSpaceObjectDestroyed(missile))
        continue
      for (asteroid in this.asteroids) {
        if (this.wasSpaceObjectDestroyed(asteroid))
          continue
        if (missile.impacts(asteroid)) {
          this.generateExplosion(asteroid, missile)
          this.destroySpaceObject(asteroid)
          this.destroySpaceObject(missile)
        }
      }
    }
  }
  
  fun wasSpaceObjectDestroyed(spaceObject: SpaceObject): Boolean {
    var outsideBoundariesPosition = Point2D(
      width + spaceObject.radius, height + spaceObject.radius
    )
    var stopVelocity = Vector2D(0.,0.)
    if (
      spaceObject.center == outsideBoundariesPosition &&
      spaceObject.velocity == stopVelocity
    )
      return true
    else 
      return false
  ...
  fun destroySpaceObject(spaceObject: SpaceObject) {
    var outsideBoundariesPosition = Point2D(
      width + spaceObject.radius, height + spaceObject.radius
    )
    spaceObject.setCenter(outsideBoundariesPosition)
    var stopVelocity = Vector2D(0.,0.)
    spaceObject.setVelocity(stopVelocity)
  }

  fun disableAlreadyTriggeredExplosions() {
